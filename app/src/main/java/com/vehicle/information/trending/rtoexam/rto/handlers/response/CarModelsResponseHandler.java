...
public class CarModelsResponseHandler implements TaskHandler.JsonResponseHandler {
    private TaskHandler.ResponseHandler<CarModelsResponse> responseHandler;

    public CarModelsResponseHandler(TaskHandler.ResponseHandler<CarModelsResponse> responseHandler2) {
    ...
    public void onResponse(JSONObject jSONObject) {
        try {
            this.responseHandler.onResponse((CarModelsResponse) new Gson().fromJson(jSONObject.toString(), CarModelsResponse.class));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
