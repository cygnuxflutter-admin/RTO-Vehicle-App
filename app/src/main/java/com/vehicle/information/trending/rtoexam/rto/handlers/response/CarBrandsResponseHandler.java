...
public class CarBrandsResponseHandler implements TaskHandler.JsonResponseHandler {
    private TaskHandler.ResponseHandler<CarBrandsResponse> responseHandler;

    public CarBrandsResponseHandler(TaskHandler.ResponseHandler<CarBrandsResponse> responseHandler2) {
    ...
    public void onResponse(JSONObject jSONObject) {
        try {
            this.responseHandler.onResponse((CarBrandsResponse) new Gson().fromJson(jSONObject.toString(), CarBrandsResponse.class));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
