...
public class Task_ResultLstAdpter extends BaseAdapter {
    ...
    public Task_ResultLstAdpter(Context context, int resource, ArrayList<HashMap<String, Object>> arrayList, ArrayList<HashMap<String, Integer>> arrayList2, ArrayList<Integer> arrayList3, ArrayList<Integer> arrayList4, String str) {
        ...
        if (context != null) {
            ...
            int prefInterval = pref.getAdsStatus("NativeAdIntervalCount");
            this.interval = prefInterval > 0 ? prefInterval : 10;
    ...
    @Override
    public int getCount() {
        if (this.originalValues.isEmpty()) return 0;
        if (!isAdEnabled || interval <= 0) return this.originalValues.size();
        int totalAds = this.originalValues.size() / interval;
        return this.originalValues.size() + totalAds;
    }
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        if (viewType == VIEW_TYPE_AD) {
            ...
            if (nativeAdContainer != null && context instanceof Activity) {
                Task_NativeAdUtil.loadNativeAd(nativeAdContainer, (Activity) context);
            }
            return view;
        }
        ...
        if (realPos < 0 || realPos >= this.originalValues.size()) {
            return view;
        }

        HashMap<String, Object> map = this.originalValues.get(realPos);
        int num = (this.number != null && realPos < this.number.size()) ? this.number.get(realPos) : (realPos + 1);
        ...
        if ("photo".equals(photoVal)) {
            try {
                int resId = 0;
                if (this.Image != null && realPos < this.Image.size()) {
                    int imgIdx = this.Image.get(realPos);
